...
.factory('posRules', ['$http', 'promiseTracker', '$injector', function($http) {
	return {
		get : function() {
			return $http.get('/api/posRule');
		},
		create : function(rulesData) {
			return $http.post('/api/posRule', rulesData);
		},
		delete : function(id) {
			return $http.delete('/api/posRule/' + id);
		}
	}
}])
