function initialize() {
  /* Setup Google Map  -- Commment it to replace with MapQuest
  ...
  // Setup Mapquest options
  // create an object for options
  var options = {
    elt: document.getElementById('map_canvas'),           // ID of map element on page
    zoom: 4,                                      // initial zoom level of the map
    latLng: { lat: 17.7850, lng: -12.4183 },  // center of map in latitude/longitude
    mtype: 'map',                                  // map type (map, sat, hyb); defaults to map
    bestFitMargin: 1,                              // margin offset from map viewport when applying a bestfit on shapes
    ...
    socket.on('twitter-stream', function (data) {
      ...
      */
      ...
      MQA.withModule('shapes', function() {
        ...
        circle.radius='0.3';
        circle.color='#FA5858';
        circle.shapePoints=[data.lng, data.lat];
        //var basic=new MQA.Poi( {lng:data.lng, lat:data.lat} );
        map.addShape(circle);
        map.bestFit();
      });
